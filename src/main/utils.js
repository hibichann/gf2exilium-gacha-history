 ...
 *
 * @param {*} url 抽卡日志的URL
 * @param {*} body 抽卡日志的POST Payload
 * @returns
 */
const request = async (urlAndAccessToken, body) => {
  const res = await fetch(urlAndAccessToken.url, {
    method: "POST",
    body: body,
    headers: {
      Authorization: urlAndAccessToken.accessToken,
      "User-Agent":
...
async function getCacheText(gamePath) {
  ...
  const cacheBytes = await fs
    .readFile(path.join(timeSortedFiles[0]))
    .filter((x) => x !== 0x00);
  const cacheText = Buffer.from(cacheBytes);
